...
const Layout: React.FC<LayoutProps> = ({ 
  ...
  return (
    <div className={cn(
      ...
      <header className="sticky top-0 z-50 w-full border-b bg-white/95 backdrop-blur supports-[backdrop-filter]:bg-white/60 dark:bg-slate-900/95 dark:supports-[backdrop-filter]:bg-slate-900/60">
        <div className="container flex h-14 items-center justify-between px-4 sm:px-6 lg:px-8">
          ...
          <nav className="hidden md:flex items-center gap-6 text-sm font-medium">
            <a href="#" className="transition-colors hover:text-slate-900 text-slate-600 dark:text-slate-300 dark:hover:text-slate-100">
              Templates
            </a>
            <a href="#" className="transition-colors hover:text-slate-900 text-slate-600 dark:text-slate-300 dark:hover:text-slate-100">
              Examples
      ...
      {showControls && (
        <ControlsBar className="sticky top-14 z-40" />
      )}
